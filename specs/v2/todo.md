...
The crokcode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@crokcode/llm` tool loop after replacing its
...
- plugins get global "crokcode" instance like in that post i showed
- crokcode instance has stuff like `crokcode.session.prompt()` or
  `crokcode.tool.register({...})`
...
- decide whether to preserve deprecated `@crokcode/llm` orchestration exports
