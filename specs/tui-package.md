...
Move the canonical CrokCode terminal application from
`packages/crokcode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/crokcode`,
`packages/cli`, or `@crokcode/core`.

The SDK is the TUI's CrokCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/crokcode`.
- Do not replace private `packages/crokcode` imports with `@crokcode/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@crokcode/tui`.
...
- CrokCode domain data displayed by the TUI
...
```text
packages/crokcode/src/cli/cmd/tui
```

Its private dependency on `packages/crokcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/crokcode/src/*`.
...
Status: Completed. The private `@crokcode/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@crokcode/tui`.
...
- The package has no dependency on `crokcode`, `@crokcode/cli`, or
  `@crokcode/core`.
...
primitives now live in `@crokcode/tui`. The legacy host consumes explicit
...
and build inputs through `@crokcode/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads CrokCode globals or process state;
...
Replace process-global CrokCode state with resolved TUI inputs.
...
`@crokcode/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/crokcode` initially.
...
canonical in `@crokcode/tui`. Configured references resolve through the new
...
adapters rather than CrokCode domain implementations.

Make the SDK the only CrokCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports CrokCode data and operations only from
  `@crokcode/sdk`.
...
`@crokcode/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/crokcode`.
...
- The package has no imports from `packages/crokcode`, `packages/cli`, or
  `@crokcode/core`.
...
public `@crokcode/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/crokcode`.
- Keep the legacy embedded worker and server startup in `packages/crokcode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@crokcode/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/crokcode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@crokcode/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@crokcode/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for CrokCode domain state.
...
cd packages/crokcode && bun typecheck
...
rg '@crokcode/core|packages/crokcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/crokcode/src packages/crokcode/test
```
...
- Build the current-platform `packages/crokcode` binary.
