# Remove `packages/crokcode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/crokcode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the crokcode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/crokcode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/crokcode/src/account/repo.ts`
- `packages/crokcode/src/cli/cmd/db.ts`
- `packages/crokcode/src/cli/cmd/import.ts`
- `packages/crokcode/src/cli/cmd/stats.ts`
- `packages/crokcode/src/control-plane/workspace.ts`
- `packages/crokcode/src/index.ts`
- `packages/crokcode/src/node.ts`
- `packages/crokcode/src/permission/index.ts`
- `packages/crokcode/src/project/project.ts`
- `packages/crokcode/src/server/projectors.ts`
- `packages/crokcode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/crokcode/src/server/shared/fence.ts`
- `packages/crokcode/src/session/message-v2.ts`
- `packages/crokcode/src/session/projectors.ts`
- `packages/crokcode/src/session/prompt.ts`
- `packages/crokcode/src/session/session.ts`
- `packages/crokcode/src/session/todo.ts`
- `packages/crokcode/src/share/share-next.ts`
- `packages/crokcode/src/storage/db.ts`
- `packages/crokcode/src/sync/index.ts`
- `packages/crokcode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy crokcode database wrapper; `packages/crokcode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the crokcode wrapper.
...
Status: Completed. `SyncEvent` and the crokcode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy crokcode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy crokcode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy crokcode database wrapper.
...
All migration groups are complete or superseded. `packages/crokcode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No crokcode data-migration group remains.

The previous crokcode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate crokcode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/crokcode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/crokcode/src`
- `bun typecheck` from `packages/crokcode`
- Relevant package tests from `packages/crokcode`, not the repo root
