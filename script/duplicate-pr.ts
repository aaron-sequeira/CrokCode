...
import { createCrokcode } from "@crokcode/sdk"
...
`)
  ...
  const crokcode = await createCrokcode({ port: 0 })
  ...
  try {
    ...
    const session = await crokcode.client.session.create()
    const result = await crokcode.client.session
      .prompt({
