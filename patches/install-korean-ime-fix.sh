...
if [ -d "$CROKCODE_SRC/.git" ]; then
  info "Updating existing source at $CROKCODE_SRC ..."
  git -C "$CROKCODE_SRC" fetch origin "$FORK_BRANCH"
  git -C "$CROKCODE_SRC" checkout "$FORK_BRANCH"
  git -C "$CROKCODE_SRC" reset --hard "origin/$FORK_BRANCH"
else
  info "Cloning fork (shallow) to $CROKCODE_SRC ..."
  git clone --depth 1 --branch "$FORK_BRANCH" "$FORK_REPO" "$CROKCODE_SRC"
fi
