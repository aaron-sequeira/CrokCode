...
> current private `@crokcode/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@crokcode/ai`
...
- Secondary audience: CrokCode and other durable agent runtimes
...
import { LLM } from "@crokcode/ai"
import { OpenAI } from "@crokcode/ai/providers/openai"
...
```ts
import { Message } from "@crokcode/ai"
...
import { LLM, Tool } from "@crokcode/ai"
...
CrokCode and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@crokcode/ai/promise"
import { OpenAI } from "@crokcode/ai/providers/openai"
...
```ts
import { LLMSchema } from "@crokcode/ai/schema"
...
```ts
import { Provider, Protocol } from "@crokcode/ai/provider"
...
@crokcode/ai/provider
  experimental Provider and Protocol authoring APIs

@crokcode/ai/providers/openai
@crokcode/ai/providers/anthropic
@crokcode/ai/providers/google
...
```
...
CrokCode should migrate to `generateTurn` / `streamTurn`, preserving its durable
