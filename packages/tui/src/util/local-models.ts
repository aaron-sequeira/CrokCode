...
export const LOCAL_MODELS: LocalModel[] = [
  ...
  { id: "qwen2.5-coder:7b", name: "Qwen2.5 Coder", params: "7B", sizeGb: 4.7, minRamGb: 8, note: "Great all-rounder" },
  { id: "llama3.1:8b", name: "Llama 3.1", params: "8B", sizeGb: 4.9, minRamGb: 8, note: "General purpose" },
...
export async function ollamaStatus(): Promise<{ running: boolean; installed: Set<string> }> {
  try {
    const res = await fetch(`${OLLAMA_URL}/api/tags`, { signal: AbortSignal.timeout(2000) })
    if (!res.ok) return { running: false, installed: new Set() }
    const body = (await res.json()) as { models?: { name: string }[] }
    return { running: true, installed: new Set((body.models ?? []).map((m) => m.name)) }
  } catch {
    return { running: false, installed: new Set() }
  }
}
