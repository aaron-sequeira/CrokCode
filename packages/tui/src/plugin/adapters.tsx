import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@crokcode/plugin/tui"
...
import type { useCrokcodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useCrokcodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getCrokcodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getCrokcodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
