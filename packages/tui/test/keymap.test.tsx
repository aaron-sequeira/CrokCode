...
import { getCrokcodeModeStack, CROKCODE_BASE_MODE, CrokcodeKeymapProvider, registerCrokcodeKeymap } from "../src/keymap"
...
test("mode-less bindings stay active when crokcode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getCrokcodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getCrokcodeModeStack(keymap).push("autocomplete")
