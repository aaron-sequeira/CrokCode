...
Создайте `.crokcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию crokcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем crokcode [config](/docs/config):
...
- Глобальный: `~/.config/crokcode/commands/`
- Для каждого проекта: `.crokcode/commands/`
...
crokcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
