---
title: OpenCode Zen
description: CrokCode によって提供されるモデルの厳選されたリスト。
---
...
OpenCode Zen は、CrokCode チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は CrokCode のほかのプロバイダーと同じように動作します。OpenCode Zen にログインして API キーを取得できます。これは **完全にオプション** であり、CrokCode を使うために必須ではありません。
...
:::tip
CrokCode でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
OpenCode Zen は、CrokCode のほかのプロバイダーと同じように動作します。
...
CrokCode 設定で使う [model id](/docs/config/#models) は `crokcode/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `crokcode/gpt-5.5` を使用します。
...
- DeepSeek V4 Flash Free は期間限定で CrokCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で CrokCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- North Mini Code Free は期間限定で CrokCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で CrokCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で CrokCode で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに CrokCode では常にほかのプロバイダーも使えるようにすること。
