...
Wenn CrokCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\crokcode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `crokcode --log-level DEBUG`.

---

## Speicher

crokcode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/crokcode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\crokcode`
...
CrokCode Desktop startet im Hintergrund einen lokalen CrokCode-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `CrokCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/crokcode/crokcode.jsonc` (oder `~/.config/crokcode/crokcode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/crokcode/crokcode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\crokcode\crokcode.jsonc`
...
CrokCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/crokcode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\crokcode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit CrokCode ihn neu aufbauen kann.

1. Beende CrokCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/crokcode`
- **Linux**: loesche `~/.cache/crokcode` (oder fuehre aus: `rm -rf ~/.cache/crokcode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\crokcode`

3. Starte CrokCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

CrokCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `crokcode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CROKCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CROKCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt CrokCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit CrokCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

CrokCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer CrokCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende CrokCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von CrokCode Desktop):

- `crokcode.settings.dat` (desktop default server URL)
- `crokcode.global.dat` und `crokcode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit CrokCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `crokcode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\crokcode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. crokcode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\crokcode`

2. Starte crokcode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt crokcode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
crokcode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
