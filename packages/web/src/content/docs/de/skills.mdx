...
Agent Skills erlauben CrokCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
CrokCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft CrokCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.crokcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/crokcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.crokcode/skills/git-release/SKILL.md` zum Beispiel so:
...
CrokCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `crokcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `crokcode.json`):
