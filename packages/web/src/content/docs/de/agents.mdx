...
Es gibt zwei Arten von Agenten in CrokCode; Primäragenten und Subagenten.
...
CrokCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `crokcode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet CrokCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale CrokCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer CrokCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenCode Zen](/docs/zen) verwenden, würden Sie `crokcode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `crokcode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
