---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um CrokCode zu erweitern.
---

Mit Plugins können Sie CrokCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von CrokCode zu ändern.
...
- `.crokcode/plugins/` – Plugins auf Projektebene
- `~/.config/crokcode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/crokcode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/crokcode/crokcode.json`)
2. Projektkonfiguration (`crokcode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/crokcode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.crokcode/plugins/`)
...
CrokCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein CrokCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie CrokCode erweitern können.
...
:::note
Wenn Sie die CrokCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass CrokCode `.env`-Dateien liest:
...
Plugins können CrokCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das CrokCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für CrokCode zur Verfügung.
