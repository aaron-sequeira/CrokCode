...
Mit CrokCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet CrokCode das `crokcode`-Theme.
...
CrokCode liefert mehrere eingebaute Themes mit.
...
- CrokCode optisch ans Terminal anpassen wollen
...
CrokCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/crokcode/themes/*.json` oder `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.crokcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.crokcode/themes/*.json`
