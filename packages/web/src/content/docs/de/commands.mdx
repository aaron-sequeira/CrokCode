...
Erstellen Sie `.crokcode/commands/test.md`:

```md title=".crokcode/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die CrokCode-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem CrokCode [config](/docs/config):

```json title="crokcode.jsonc" {4-12}
{
...
- Global: `~/.config/crokcode/commands/`
- Pro Projekt: `.crokcode/commands/`

```markdown title="~/.config/crokcode/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".crokcode/commands/component.md"
---
...
```md title=".crokcode/commands/create-file.md"
---
...
```md title=".crokcode/commands/analyze-coverage.md"
---
...
```md title=".crokcode/commands/review-changes.md"
---
...
```md title=".crokcode/commands/review-component.md"
---
...
CrokCode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
