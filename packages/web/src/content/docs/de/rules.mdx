---
title: Regeln
description: Lege eigene Anweisungen fuer crokcode fest.
---

Du kannst crokcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in crokcode den Befehl `/init` aus.
...
So kann crokcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht crokcode sie zu erweitern.
...
crokcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/crokcode/AGENTS.md` speichern.
Diese gelten in allen crokcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt CrokCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/crokcode/AGENTS.md` existiert)
...
Beim Start sucht crokcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/crokcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `crokcode.json` oder in `~/.config/crokcode/crokcode.json` angeben.
...
crokcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit crokcode.json

Empfohlen ist das `instructions`-Feld in der `crokcode.json`:
...
Du kannst crokcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `crokcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
