---
title: Policies
description: Control which configured resources CrokCode may use.
---

Policies control whether CrokCode may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `crokcode.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether CrokCode may use a resource such as an LLM provider.
...
CrokCode currently supports one policy action:
