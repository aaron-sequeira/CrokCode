...
crokcode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\crokcode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `crokcode --log-level DEBUG`.

---

## Depolama

crokcode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/crokcode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\crokcode` yapistirin
...
crokcode Desktop arka planda yerel bir crokcode sunucusu (`opencode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `crokcode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/crokcode/crokcode.jsonc` (veya `~/.config/crokcode/crokcode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/crokcode/crokcode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\crokcode\crokcode.jsonc` yapistirin
...
crokcode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/crokcode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\crokcode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), crokcode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. crokcode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/crokcode`
- **Linux**: `~/.cache/crokcode` dizinini silin (veya `rm -rf ~/.cache/crokcode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\crokcode` yapistirin

3. crokcode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

crokcode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`crokcode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `CROKCODE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `CROKCODE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta crokcode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, crokcode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

crokcode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda crokcode icin bildirimler etkinse
...
1. crokcode Desktop'u kapatin
2. Su dosyalari bulun ve silin (crokcode Desktop uygulama veri dizininde yer alirlar):

- `crokcode.settings.dat` (desktop varsayilan sunucu URL'si)
- `crokcode.global.dat` ve `crokcode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
crokcode ile ilgili bir sorun yasiyorsaniz:
...
3. `crokcode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `crokcode models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\crokcode`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. crokcode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\crokcode`

2. En guncel provider paketlerini yeniden kurmak icin crokcode'u yeniden baslatin

Bu, crokcode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
crokcode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
