...
crokcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak crokcode kendi `crokcode` temasını kullanır.
...
crokcode birden fazla yerleşik temayla gelir.
...
- crokcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
crokcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/crokcode/themes/*.json` veya `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.crokcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.crokcode/themes/*.json`
