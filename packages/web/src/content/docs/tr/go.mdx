...
Go, CrokCode'daki diğer sağlayıcılar gibi çalışır. OpenCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve CrokCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenCode Go, CrokCode'daki diğer sağlayıcılar gibi çalışır.
...
CrokCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `crokcode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `crokcode-go/kimi-k3` kullanmalısınız.
...
CrokCode Go'yu şu amaçlarla oluşturduk:
...
4. CrokCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
