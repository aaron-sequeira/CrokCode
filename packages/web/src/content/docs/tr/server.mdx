---
title: Sunucu
description: crokcode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`crokcode serve` komutu, crokcode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `crokcode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `CROKCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `crokcode` değeridir; değiştirmek isterseniz `CROKCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `crokcode serve` hem de `crokcode web` için geçerlidir.
...
`crokcode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
crokcode ile programatik etkileşim için crokcode sunucusunu kullanın.
:::

Bu mimari, crokcode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`crokcode serve` ile bağımsız bir sunucu başlatabilirsiniz. crokcode TUI açıksa, `crokcode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum crokcode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
crokcode sunucusu aşağıdaki API'leri sunar.
