...
Ajan becerileri, crokcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
crokcode su konumlari tarar:

- Proje konfigurasyonu: `.crokcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/crokcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda crokcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.crokcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/crokcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.crokcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
crokcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`crokcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`crokcode.json` icinde):
