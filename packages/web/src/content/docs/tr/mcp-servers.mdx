...
_Model Bağlam Protokolü_ veya MCP kullanarak crokcode'a harici araçlar ekleyebilirsiniz. crokcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [crokcode Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
crokcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde crokcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa crokcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `crokcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından crokcode, belirteçleri `~/.local/share/crokcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra crokcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi crokcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve crokcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
