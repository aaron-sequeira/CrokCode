---
title: Eklentiler
description: CrokCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek CrokCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya CrokCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.crokcode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/crokcode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/crokcode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/crokcode/crokcode.json`)
2. Project config (`crokcode.json`)
3. Global eklenti dizini (`~/.config/crokcode/plugins/`)
4. Proje eklenti dizini (`.crokcode/plugins/`)
...
CrokCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir CrokCode SDK istemcisi.
...
CrokCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
CrokCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
CrokCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca CrokCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, CrokCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra CrokCode'a da hazır olacaktır.
