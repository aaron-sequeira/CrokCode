...
`.crokcode/commands/test.md` oluştur:
...
crokcode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

crokcode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/crokcode/commands/`
- Proje başına: `.crokcode/commands/`
...
crokcode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
