...
Puoi aggiungere strumenti esterni a CrokCode usando il _Model Context Protocol_, o MCP. CrokCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [CrokCode Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
CrokCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, CrokCode:
...
Se il server richiede autenticazione, CrokCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `crokcode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, CrokCode memorizzera' i token in modo sicuro in `~/.local/share/crokcode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in CrokCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione CrokCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare CrokCode al tuo account Sentry.
