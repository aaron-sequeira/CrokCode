...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `CROKCODE_SERVER_USERNAME` o `opencode`) |
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `CROKCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `CROKCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| `CROKCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
