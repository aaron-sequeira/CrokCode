...
Per diagnosticare problemi con CrokCode, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\crokcode\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `crokcode --log-level DEBUG`.

---

## Archiviazione

crokcode salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/crokcode/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\crokcode`
...
CrokCode Desktop esegue in background un server locale di CrokCode (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `CrokCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/crokcode/crokcode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\crokcode\crokcode.jsonc`
...
CrokCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/crokcode/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\crokcode\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che CrokCode possa ricostruirla.

1. Chiudi completamente CrokCode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/crokcode`
- **Linux**: elimina `~/.cache/crokcode` (oppure esegui `rm -rf ~/.cache/crokcode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\crokcode`

3. Riavvia CrokCode Desktop.

---

### Risolvi problemi di connessione al server

CrokCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `crokcode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `CROKCODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `CROKCODE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, CrokCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di CrokCode.

---

### Notifiche non visualizzate

CrokCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per CrokCode nelle impostazioni del sistema operativo, e
...
1. Chiudi CrokCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app CrokCode Desktop):

- `crokcode.settings.dat` (desktop default server URL)
- `crokcode.global.dat` e `crokcode.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con CrokCode:
...
Per capire a quali modelli hai accesso, esegui `crokcode models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\crokcode`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. crokcode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\crokcode`

2. Riavvia crokcode per reinstallare i pacchetti provider piu recenti

Questo forzera crokcode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
crokcode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
