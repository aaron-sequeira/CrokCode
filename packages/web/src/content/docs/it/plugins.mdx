---
title: Plugin
description: Scrivi plugin per estendere CrokCode.
---

I plugin ti permettono di estendere CrokCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di CrokCode.
...
- `.crokcode/plugins/` - plugin a livello progetto
- `~/.config/crokcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/crokcode/node_modules/`.
...
1. Config globale (`~/.config/crokcode/crokcode.json`)
2. Config di progetto (`crokcode.json`)
3. Directory plugin globale (`~/.config/crokcode/plugins/`)
4. Directory plugin di progetto (`.crokcode/plugins/`)
...
CrokCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di crokcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere crokcode.
...
:::note
Se usi l'app desktop di CrokCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a crokcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a crokcode:
...
L'helper `tool` crea uno strumento personalizzato che crokcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in crokcode insieme agli strumenti integrati.
