...
Le skill degli agenti permettono a CrokCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
CrokCode cerca in queste posizioni:

- Config di progetto: `.crokcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/crokcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, CrokCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.crokcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/crokcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.crokcode/skills/git-release/SKILL.md` cosi':
...
CrokCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `crokcode.json`:
...
**Per agenti integrati** (in `crokcode.json`):
