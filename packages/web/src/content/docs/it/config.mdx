...
description: Usa la configurazione JSON di CrokCode.
---

Puoi configurare CrokCode usando un file di configurazione JSON.

---

## Formato

CrokCode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
2. **Config globale** (`~/.config/crokcode/crokcode.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `CROKCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`crokcode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.crokcode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `CROKCODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.crokcode` e `~/.config/crokcode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di CrokCode in `~/.config/crokcode/crokcode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/crokcode/tui.json`.
...
Aggiungi `crokcode.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando CrokCode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `CROKCODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `CROKCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.crokcode` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.crokcode`, quindi **puo sovrascrivere** le loro impostazioni.
...
Usa `CROKCODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `crokcode.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `crokcode serve` e `crokcode web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server CrokCode.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `crokcode.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in CrokCode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, CrokCode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/crokcode/agents/` o `.crokcode/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, CrokCode fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`crokcode run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/crokcode/commands/` o `.crokcode/commands/`. [Scopri di piu](/docs/commands).
...
CrokCode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, crokcode **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono CrokCode con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.crokcode/plugins/` o `~/.config/crokcode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare CrokCode a usare solo alcuni provider invece di disabilitarli uno per uno.
