...
Crea `.crokcode/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di CrokCode o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di CrokCode:
...
- Globale: `~/.config/crokcode/commands/`
- Per progetto: `.crokcode/commands/`
...
crokcode include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
