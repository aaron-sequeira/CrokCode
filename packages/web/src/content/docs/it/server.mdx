---
title: Server
description: Interagisci con il server crokcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `crokcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client crokcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `crokcode.local` |
...
Imposta `CROKCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `crokcode`, oppure imposta `CROKCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `crokcode serve` sia per `crokcode web`.
...
Quando esegui `crokcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server crokcode per interagire con crokcode in modo programmatico.
:::

Questa architettura permette a crokcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `crokcode serve` per avviare un server standalone. Se la TUI di crokcode e gia in esecuzione, `crokcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di CrokCode.
...
Il server crokcode espone le seguenti API.
