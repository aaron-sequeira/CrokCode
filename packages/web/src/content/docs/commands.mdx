...
Create `.crokcode/commands/test.md`:
...
You can add custom commands through the CrokCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your CrokCode [config](/docs/config):
...
crokcode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
