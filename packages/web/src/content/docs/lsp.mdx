---
title: LSP Servers
description: CrokCode integrates with your LSP servers.
---

CrokCode can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

CrokCode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CROKCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and crokcode opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your crokcode config.
