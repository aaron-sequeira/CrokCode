...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في crokcode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في crokcode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.crokcode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/crokcode/tools/`.
