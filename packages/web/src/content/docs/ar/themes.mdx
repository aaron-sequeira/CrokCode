...
مع CrokCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم CrokCode سمتنا `crokcode`.
...
يأتي CrokCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق CrokCode مظهر terminal لديهم
...
يدعم CrokCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/crokcode/themes/*.json` أو `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.crokcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.crokcode/themes/*.json`
