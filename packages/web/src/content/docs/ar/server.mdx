---
title: الخادم
description: تفاعل مع خادم crokcode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `crokcode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل crokcode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `crokcode.local` |
...
عيّن `CROKCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `crokcode`، أو عيّن `CROKCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `crokcode serve` و `crokcode web`.
...
عند تشغيل `crokcode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم crokcode للتفاعل مع crokcode برمجيا.
:::

تتيح هذه البنية لـ crokcode دعم عدة عملاء وتمكّنك من التفاعل مع crokcode برمجيا.

يمكنك تشغيل `crokcode serve` لبدء خادم مستقل. إذا كانت واجهة crokcode في terminal (TUI)
قيد التشغيل، فسيبدأ `crokcode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات CrokCode لـ [IDE](/docs/ide).
...
يعرض خادم crokcode واجهات API التالية.
