...
Go fonctionne comme n'importe quel autre fournisseur dans CrokCode. Vous vous abonnez à OpenCode Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser CrokCode.
...
OpenCode Go fonctionne comme n'importe quel autre fournisseur dans CrokCode.
...
L'[ID de modèle](/docs/config/#models) dans votre configuration CrokCode utilise le format `opencode-go/<model-id>`. Par exemple, pour Kimi K3, vous utiliseriez `opencode-go/kimi-k3` dans votre configuration.
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec CrokCode également.
