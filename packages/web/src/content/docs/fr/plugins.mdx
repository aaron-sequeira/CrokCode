---
title: Plugins
description: Écrivez vos propres plugins pour étendre CrokCode.
---

Les plugins vous permettent d'étendre CrokCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de CrokCode.
...
- `.crokcode/plugins/` - Plugins au niveau du projet
- `~/.config/crokcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/crokcode/node_modules/`.
...
1. Configuration globale (`~/.config/crokcode/crokcode.json`)
2. Configuration du projet (`crokcode.json`)
3. Répertoire global des plugins (`~/.config/crokcode/plugins/`)
4. Répertoire des plugins du projet (`.crokcode/plugins/`)
...
CrokCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK crokcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre crokcode.
...
:::note
Si vous utilisez l'application de bureau CrokCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez crokcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à crokcode :
...
L'assistant `tool` crée un outil personnalisé que crokcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour crokcode aux côtés des outils intégrés.
