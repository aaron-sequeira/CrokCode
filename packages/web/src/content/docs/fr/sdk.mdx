---
title: SDK
description: Client JS de type sécurisé pour le serveur crokcode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK crokcode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler crokcode par programme.
...
Créez une instance de crokcode :

```javascript
import { createCrokcode } from "@crokcode/sdk"

const { client } = await createCrokcode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `crokcode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${crokcode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de crokcode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@crokcode/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*crokcode" },
})
