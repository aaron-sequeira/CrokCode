...
Avec CrokCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, CrokCode utilise notre propre thème `crokcode`.
...
CrokCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que CrokCode corresponde à l'apparence de leur terminal
...
CrokCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/crokcode/themes/*.json` ou `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.crokcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.crokcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/crokcode/themes
vim ~/.config/crokcode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .crokcode/themes
vim .crokcode/themes/my-theme.json
```
