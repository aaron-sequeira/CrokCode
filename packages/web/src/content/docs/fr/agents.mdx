...
Il existe deux types d'agents dans CrokCode ; agents primaires et sous-agents.
...
CrokCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
CrokCode est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

CrokCode est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `crokcode.json` :
...
Si aucune température n'est spécifiée, CrokCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale CrokCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration CrokCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenCode Zen](/docs/zen), vous utiliserez `crokcode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `crokcode models` pour voir une liste des modèles disponibles.
:::
