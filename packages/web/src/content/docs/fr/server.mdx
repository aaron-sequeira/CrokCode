---
title: Serveur
description: Interagissez avec le serveur crokcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `crokcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client crokcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `crokcode.local` |
...
Définissez `CROKCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `crokcode`, ou définissez `CROKCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `crokcode serve` et à `crokcode web`.
...
Lorsque vous exécutez `crokcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur crokcode pour interagir avec crokcode par programmation.
:::

Cette architecture permet à crokcode de prendre en charge plusieurs clients et vous permet d'interagir avec crokcode par programme.

Vous pouvez exécuter `crokcode serve` pour démarrer un serveur autonome. Si vous avez le
crokcode TUI en cours d'exécution, `crokcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins CrokCode [IDE](/docs/ide).
...
Le serveur crokcode expose les API suivantes.
