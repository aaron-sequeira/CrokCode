...
As habilidades do agente permitem que o crokcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O crokcode pesquisa nesses locais:

- Configuração do projeto: `.crokcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/crokcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o crokcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.crokcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/crokcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.crokcode/skills/git-release/SKILL.md` assim:
...
O crokcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `crokcode.json`:
...
**Para agentes embutidos** (em `crokcode.json`):
