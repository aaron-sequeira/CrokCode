...
Com o crokcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o crokcode usa nosso próprio tema `crokcode`.
...
O crokcode vem com vários temas embutidos.
...
- Querem que o crokcode corresponda à aparência do seu terminal
...
O crokcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/crokcode/themes/*.json` ou `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.crokcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.crokcode/themes/*.json`
