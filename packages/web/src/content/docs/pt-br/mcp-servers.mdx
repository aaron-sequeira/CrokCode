...
Você pode adicionar ferramentas externas ao crokcode usando o _Model Context Protocol_, ou MCP. O crokcode suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do crokcode](https://opencode.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
O crokcode lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o crokcode irá:
...
Se o servidor requer autenticação, o crokcode solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `crokcode mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o crokcode armazenará os tokens de forma segura em `~/.local/share/crokcode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no crokcode, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do crokcode como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o crokcode à sua conta do Sentry.
