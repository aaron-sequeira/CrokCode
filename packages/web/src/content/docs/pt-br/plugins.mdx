---
title: Plugins
description: Escreva seus próprios plugins para estender o crokcode.
---

Plugins permitem que você estenda o crokcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do crokcode.
...
- `.crokcode/plugins/` - Plugins em nível de projeto
- `~/.config/crokcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/crokcode/node_modules/`.
...
1. Configuração global (`~/.config/crokcode/crokcode.json`)
2. Configuração do projeto (`crokcode.json`)
3. Diretório de plugins global (`~/.config/crokcode/plugins/`)
4. Diretório de plugins do projeto (`.crokcode/plugins/`)
...
O crokcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do crokcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o crokcode.
...
:::note
Se você estiver usando o aplicativo desktop crokcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o crokcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao crokcode:
...
O helper `tool` cria uma ferramenta personalizada que o crokcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o crokcode junto com as ferramentas integradas.
