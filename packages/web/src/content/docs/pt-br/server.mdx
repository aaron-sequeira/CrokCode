---
title: Servidor
description: Interaja com o servidor crokcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `crokcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente crokcode pode usar.
...
```bash
crokcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `crokcode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
crokcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `CROKCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `crokcode`, ou defina `CROKCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `crokcode serve` quanto ao `crokcode web`.

```bash
CROKCODE_SERVER_PASSWORD=your-password crokcode serve
```
...
Quando você executa `crokcode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor crokcode para interagir com o crokcode programaticamente.
:::

Essa arquitetura permite que o crokcode suporte múltiplos clientes e permite que você interaja com o crokcode programaticamente.

Você pode executar `crokcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do crokcode em execução, `crokcode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do crokcode [IDE](/docs/ide).
...
O servidor crokcode expõe as seguintes APIs.
