...
As ferramentas permitem que o LLM execute ações em sua base de código. O crokcode vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no crokcode.
...
:::note
Esta ferramenta está disponível apenas quando `CROKCODE_EXPERIMENTAL_LSP_TOOL=true` (ou `CROKCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor crokcode ou quando a variável de ambiente `CROKCODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o crokcode:
