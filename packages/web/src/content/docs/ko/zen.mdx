---
title: Zen
description: CrokCode가 제공하는 엄선된 모델 목록.
---
...
OpenCode Zen은 CrokCode 팀이 제공하는, 테스트와 검증을 거친 모델 목록입니다.

Zen은 CrokCode의 다른 provider와 똑같이 작동합니다. OpenCode Zen에 로그인해 API 키를 받으면 됩니다. 이는 **완전히 선택 사항**이며, CrokCode를 사용하기 위해 반드시 사용할 필요는 없습니다.
...
:::tip
CrokCode에서 잘 작동하는 일부 모델과 provider를 직접 테스트했습니다.
:::
...
OpenCode Zen은 CrokCode의 다른 provider와 똑같이 작동합니다.
...
CrokCode config에서 사용하는 [모델 ID](/docs/config/#models)는 `crokcode/<model-id>` 형식입니다. 예를 들어 GPT 5.5를 사용하려면 config에서 `crokcode/gpt-5.5`를 사용하면 됩니다.
...
- DeepSeek V4 Flash Free는 한정된 기간 동안 CrokCode에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- MiMo-V2.5 Free는 한정된 기간 동안 CrokCode에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- North Mini Code Free는 한정된 기간 동안 CrokCode에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- Nemotron 3 Ultra Free는 한정된 기간 동안 CrokCode에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- Big Pickle은 한정된 기간 동안 CrokCode에서 무료로 제공되는 stealth model입니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
...
4. 다른 코딩 에이전트와도 함께 사용할 수 있게 해 **lock-in이 없도록** 하기 위해. 또한 CrokCode에서 언제나 다른 provider도 사용할 수 있게 하기 위해.
