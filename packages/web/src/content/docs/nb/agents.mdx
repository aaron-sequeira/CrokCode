...
Det er to typer agenter i CrokCode; primære agenter og underagenter.
...
CrokCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
CrokCode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

CrokCode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `crokcode.json`:
...
Hvis ingen temperatur er spesifisert, bruker CrokCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CrokCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i CrokCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenCode Zen](/docs/zen), vil du bruke `crokcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `crokcode models` for å se en liste over tilgjengelige modeller.
:::
