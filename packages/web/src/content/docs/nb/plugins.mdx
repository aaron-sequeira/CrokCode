...
description: Skriv dine egne programtillegg for å utvide CrokCode.
---

Plugins lar deg utvide CrokCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til CrokCode.
...
- `.crokcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/crokcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/crokcode/crokcode.json`)
2. Prosjektkonfigurasjon (`crokcode.json`)
3. Global plugin-katalog (`~/.config/crokcode/plugins/`)
4. Prosjektpluginkatalog (`.crokcode/plugins/`)
...
CrokCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En CrokCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide CrokCode.
...
:::note
Hvis du bruker CrokCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre CrokCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til CrokCode:
...
`tool`-hjelperen lager et tilpasset verktøy som CrokCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for CrokCode sammen med innebygde verktøy.
