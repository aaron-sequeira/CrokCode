---
title: Tjener
description: Samhandle med CrokCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `crokcode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en CrokCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `crokcode.local` |
...
Sett `CROKCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `crokcode` som standard, eller sett `CROKCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `crokcode serve` og `crokcode web`.
...
Når du kjører `crokcode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk CrokCode-serveren til å samhandle med CrokCode programmatisk.
:::

Denne arkitekturen lar CrokCode støtte flere klienter og lar deg samhandle med CrokCode programmatisk.

Du kan kjøre `crokcode serve` for å starte en frittstående server. Hvis du har
CrokCode TUI kjører, vil `crokcode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av CrokCode [IDE](/docs/ide) plugins.
...
crokcode-serveren viser følgende APIer.
