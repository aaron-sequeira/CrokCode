...
Du kan legge til eksterne verktøy i CrokCode ved å bruke _Model Context Protocol_ eller MCP. CrokCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [CrokCode Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="crokcode.jsonc" {6}
{
...
CrokCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil CrokCode:
...
Hvis serveren krever autentisering, vil CrokCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `crokcode mcp auth <server-name>`.
...
```bash
crokcode mcp auth my-oauth-server
```
...
```bash
crokcode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil CrokCode lagre tokens sikkert i `~/.local/share/crokcode/mcp-auth.json`.
...
crokcode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i CrokCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom CrokCode-konfigurasjonen som alle andre verktøy.
...
```json title="crokcode.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
crokcode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble CrokCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver CrokCode-samtale oppført som en separat økt.
