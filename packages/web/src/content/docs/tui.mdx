---
title: TUI
description: Using the CrokCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CrokCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running CrokCode starts the TUI for the current directory.
...
When using the CrokCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to CrokCode. Allows you to select from available providers and add their API keys.
...
Exit CrokCode. _Aliases_: `/quit`, `/q`
...
This is separate from `crokcode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long CrokCode waits after the leader key. Defaults to `2000`.
...
Use `CROKCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `crokcode.default`.
