---
title: Server
description: Interact with crokcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `crokcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an crokcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `crokcode.local` |
...
Set `CROKCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `crokcode`, or set `CROKCODE_SERVER_USERNAME` to override it. This applies to both `crokcode serve` and `crokcode web`.
...
When you run `crokcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the crokcode server to interact with crokcode programmatically.
:::

This architecture lets crokcode support multiple clients and allows you to interact with crokcode programmatically.

You can run `crokcode serve` to start a standalone server. If you have the
crokcode TUI running, `crokcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CrokCode [IDE](/docs/ide) plugins.
...
The crokcode server exposes the following APIs.
