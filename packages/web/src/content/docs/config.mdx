---
title: Config
description: Using the CrokCode JSON config.
---

You can configure CrokCode using a JSON config file.

---

## Format

CrokCode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="crokcode.jsonc"
{
...
2. **Global config** (`~/.config/crokcode/crokcode.json`) - user preferences
3. **Custom config** (`CROKCODE_CONFIG` env var) - custom overrides
4. **Project config** (`crokcode.json` in project) - project-specific settings
5. **`.crokcode` directories** - agents, commands, plugins
6. **Inline config** (`CROKCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/crokcode/` on macOS) - admin-controlled
...
:::note
The `.crokcode` and `~/.config/crokcode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global CrokCode config in `~/.config/crokcode/crokcode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/crokcode/tui.json`.
...
Add `crokcode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When CrokCode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `CROKCODE_CONFIG` environment variable.

```bash
export CROKCODE_CONFIG=/path/to/my/custom-config.json
crokcode run "Hello world"
```
...
Specify a custom config directory using the `CROKCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.crokcode` directory, and should
follow the same structure.

```bash
export CROKCODE_CONFIG_DIR=/path/to/my/config-directory
crokcode run "Hello world"
```

The custom directory is loaded after the global config and `.crokcode` directories, so it **can override** their settings.
...
Drop an `crokcode.json` or `crokcode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/crokcode/` |
| Linux    | `/etc/crokcode/`                         |
| Windows  | `%ProgramData%\crokcode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, CrokCode reads managed preferences from the `ai.crokcode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

CrokCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.crokcode.managed.plist`
2. `/Library/Managed Preferences/ai.crokcode.managed.plist`

The plist keys map directly to `crokcode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.crokcode.managed` PayloadType. The CrokCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.crokcode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.crokcode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.crokcode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Use `CROKCODE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `crokcode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `crokcode serve` and `crokcode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your CrokCode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `crokcode.local`. Useful for running multiple instances on the same network.
...
If not specified, CrokCode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your CrokCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, CrokCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny CrokCode actions on configured resources. Currently, policies can control which providers CrokCode may use.
...
CrokCode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, CrokCode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/crokcode/agents/` or `.crokcode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, CrokCode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`crokcode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/crokcode/commands/` or `.crokcode/commands/`. [Learn more here](/docs/commands).
...
CrokCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
CrokCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, crokcode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend CrokCode with custom tools, hooks, and integrations.

Place plugin files in `.crokcode/plugins/` or `~/.config/crokcode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="crokcode.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["crokcode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict CrokCode to only use specific providers rather than disabling them one by one.
