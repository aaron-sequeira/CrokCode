...
You can add external tools to CrokCode using the _Model Context Protocol_, or MCP. CrokCode supports both local and remote servers.
...
You can define MCP servers in your [CrokCode Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
CrokCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CrokCode will:
...
If the server requires authentication, CrokCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `crokcode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, CrokCode will store the tokens securely in `~/.local/share/crokcode/mcp-auth.json`.
...
Your MCPs are available as tools in CrokCode, alongside built-in tools. So you can manage them through the CrokCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect CrokCode to your Sentry account.
