...
CrokCode has a list of keybinds that you can customize through `tui.json`.
...
```json title="tui.json"
{
  ...
  "keybinds": {
    ...
    "editor_down": "j,down",
    "editor_up": "k,up",
    "editor_toggle": "enter,space",
    "editor_conflict_keep": "k",
    "editor_conflict_take": "t",
    "editor_conflict_diff": "d",
    "theme_list": "<leader>t",
...
CrokCode uses a `leader` key for many keybinds. This avoids conflicts in your terminal.
...
`leader_timeout` controls how long CrokCode waits for the next key after the leader key. It defaults to `2000` milliseconds.
...
The CrokCode desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `crokcode.json`.
