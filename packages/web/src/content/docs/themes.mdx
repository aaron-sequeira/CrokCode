...
With CrokCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, CrokCode uses our own `crokcode` theme.
...
CrokCode comes with several built-in themes.
...
- Want CrokCode to match their terminal's appearance
...
CrokCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/crokcode/themes/*.json` or `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.crokcode/themes/*.json`
4. **Current working directory** - Defined in `./.crokcode/themes/*.json`
