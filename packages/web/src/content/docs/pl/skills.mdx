...
Umiejętności agenta pozwalają crokcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
crokcode przeszukuje te lokalizacje:

- Project config: `.crokcode/skills/<name>/SKILL.md`
- Global config: `~/.config/crokcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu crokcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.crokcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/crokcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.crokcode/skills/git-release/SKILL.md` w ten sposób:
...
crokcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `crokcode.json`:
...
**Dla agentów wbudowanych** (w `crokcode.json`):
