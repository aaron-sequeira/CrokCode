...
Możesz dodać dodatkowe informacje do crokcode za pomocą _Model Context Protocol_, w skrócie MCP. crokcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [crokcode Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
crokcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, opencode:
...
Jeśli serwer wymaga uwierzytelnienia, crokcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `crokcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji crokcode bezpieczne przechowa tokeny w `~/.local/share/crokcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w crokcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie crokcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć crokcode z kontem Sentry.
