---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć crokcode.
---

Wtyczki umożliwiające wyłączenie crokcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania crokcode.
...
- `.crokcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/crokcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/crokcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/crokcode/crokcode.json`)
2. Konfiguracja projektu (`crokcode.json`)
3. Globalny katalog wtyczek (`~/.config/crokcode/plugins/`)
4. Katalog wtyczek projektu (`.crokcode/plugins/`)
...
crokcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z crokcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do crokcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej crokcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj crokcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do crokcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod crokcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
