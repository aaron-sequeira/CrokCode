...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. CrokCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w CrokCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CROKCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `CROKCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy CrokCode lub gdy zmienna środowiskowa `CROKCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu CrokCode:
