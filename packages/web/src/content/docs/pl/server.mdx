---
title: Serwer
description: Interakcja z serwerem crokcode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `crokcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient crokcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `crokcode.local` |
...
Ustaw `CROKCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `crokcode` lub ustaw `CROKCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `crokcode serve`, jak i `crokcode web`.
...
Kiedy uruchomisz `crokcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera crokcode do programowej interakcji z crokcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez crokcode i programową interakcję z kodem otwartym.

Możesz uruchomić `crokcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
crokcode TUI działa, `crokcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki crokcode [IDE](/docs/ide).
...
Serwer crokcode udostępnia następujące interfejsy API.
