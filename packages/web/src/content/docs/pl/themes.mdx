...
Dzięki crokcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie crokcode używa naszego własnego motywu `crokcode`.
...
crokcode ma kilka wbudowanych motywów.
...
- Chcą, aby CrokCode pasował do wyglądu ich terminala
...
crokcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/crokcode/themes/*.json` lub `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.crokcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.crokcode/themes/*.json`
