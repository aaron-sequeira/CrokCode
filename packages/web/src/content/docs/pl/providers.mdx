---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w crokcode.
---

import config from "../../../../config.mjs"
export const console = config.console

crokcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji crokcode.
...
w `~/.local/share/crokcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim crokcode
config.
...
OpenCode Zen to lista modeli dostarczonych przez zespół crokcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z crokcode. [Dowiedz się więcej](/docs/zen).
...
Działa jak każdy inny dostawca w crokcode i jest całkowicie opcjonalny w użyciu.

---

## OpenCode Go

OpenCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół crokcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z crokcode.
...
Aby używać Amazon Bedrock z crokcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania crokcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `crokcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w crokcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować crokcode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod crokcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji crokcode.
...
crokcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować crokcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`crokcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem crokcode:
...
Aby używać Google Vertex AI z crokcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania crokcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą crokcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [crokcode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację crokcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja crokcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować crokcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować crokcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować crokcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem crokcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
Aby korzystać z Ollama Cloud z crokcode:
...
3. Skopiuj klucz API do użycia w crokcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w crokcode musisz pobrać informacje o modelu lokalnie:
...
OpenCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół crokcode. [Dowiedz się więcej](/docs/zen).
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji crokcode.
...
5. Możesz je także dostosować za pomocą konfiguracji crokcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z crokcode:
...
Możesz także dostosować modele za pomocą konfiguracji crokcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z crokcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `crokcode.json` w katalogu projektu:
...
Pola `limit` pozwalają crokcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `crokcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację crokcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji crokcode.
