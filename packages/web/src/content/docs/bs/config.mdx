---
title: Konfiguracija
description: Korištenje CrokCode JSON konfiguracije.
---

Možete konfigurirati CrokCode koristeći JSON konfiguracijski fajl.

---

## Format

CrokCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
2. **Globalna konfiguracija** (`~/.config/crokcode/crokcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CROKCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`crokcode.json` u projektu) - postavke specifične za projekat
5. **`.crokcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CROKCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.crokcode` i `~/.config/crokcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu CrokCode konfiguraciju u `~/.config/crokcode/crokcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/crokcode/tui.json`.
...
Dodajte `crokcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se CrokCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CROKCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CROKCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.crokcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.crokcode`, tako da **može nadjačati** njihove postavke.
...
Koristite `CROKCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `crokcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `crokcode serve` i `crokcode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš CrokCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `crokcode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj CrokCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, CrokCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/crokcode/agents/` ili `.crokcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, CrokCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`crokcode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/crokcode/commands/` ili `.crokcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
CrokCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, CrokCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju CrokCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.crokcode/plugins/` ili `~/.config/crokcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite CrokCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
