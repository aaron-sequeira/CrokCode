...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. CrokCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u CrokCode.
...
:::note
Ovaj alat je dostupan samo kada je `CROKCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `CROKCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz CrokCode provajdera ili kada je varijabla `CROKCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju CrokCode:
