...
Agent skills omogucavaju CrokCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
CrokCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.crokcode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/crokcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, CrokCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.crokcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/crokcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.crokcode/skills/git-release/SKILL.md` ovako:
...
CrokCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `crokcode.json`:
...
**Za ugradene agente** (u `crokcode.json`):
