---
title: LSP serveri
description: CrokCode se integriše sa vašim LSP serverima.
---

CrokCode se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

CrokCode dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `CROKCODE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i crokcode otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj crokcode konfiguraciji.
