...
U CrokCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, CrokCode koristi nasu `crokcode` temu.
...
CrokCode dolazi sa vise ugradenih tema.
...
- Zele da CrokCode odgovara izgledu njihovog terminala
...
CrokCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/crokcode/themes/*.json` ili `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.crokcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.crokcode/themes/*.json`
