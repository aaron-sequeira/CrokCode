---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u CrokCode.
---

import config from "../../../../config.mjs"
export const console = config.console

CrokCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj CrokCode konfiguraciji.
...
u `~/.local/share/crokcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem CrokCode
config.
...
OpenCode Zen je lista modela koje je obezbedio CrokCode tim koji su bili
testirano i potvrđeno da dobro radi sa CrokCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite crokcode i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u CrokCode i potpuno je opcionalan za korištenje.

---

## OpenCode Go

OpenCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža CrokCode tim i koji su testirani i verificirani da dobro rade s CrokCode-om.
...
Da biste koristili Amazon Bedrock s CrokCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete crokcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `crokcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u CrokCode.
:::
...
Možete konfigurirati crokcode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi crokcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju crokcode konfiguraciju.
...
da pohrani token u crokcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
CrokCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate CrokCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`crokcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `crokcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s crokcode:
...
Za korištenje Google Vertex AI s CrokCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće crokcode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem crokcodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [crokcode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki CrokCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki CrokCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati crokcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati crokcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati crokcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za CrokCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s CrokCode:
...
3. Kopirajte API ključ za korištenje u CrokCode.
...
6. **Važno**: Prije upotrebe modela oblaka u CrokCode, morate lokalno povući informacije o modelu:
...
OpenCode Zen je lista testiranih i verifikovanih modela koju je obezbedio CrokCode tim. [Saznajte više](/docs/zen).
...
3. Unesite svoj CrokCode API ključ.
...
Također možete dodati dodatne modele putem vaše crokcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše crokcode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Crokcodeom:
...
Također možete prilagoditi modele kroz svoju crokcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s crokcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `crokcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju CrokCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `crokcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite CrokCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj crokcode konfiguraciji.
