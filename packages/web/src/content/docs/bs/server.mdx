---
title: Server
description: Komunicirajte s crokcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `crokcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji crokcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `crokcode.local` |
...
Postavite `CROKCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `crokcode`, ili postavite `CROKCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `crokcode serve` i za `crokcode web`.
...
Kada pokrenete `crokcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite crokcode server za programsku interakciju sa crokcode-om.
:::

Ova arhitektura omogucava crokcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `crokcode serve` da startate standalone server. Ako je crokcode TUI vec pokrenut, `crokcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste CrokCode [IDE](/docs/ide) pluginovi.
...
crokcode server izlaže sljedece API-je.
