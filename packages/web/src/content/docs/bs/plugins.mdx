---
title: Dodaci
description: Napišite vlastite dodatke za proširenje CrokCode.
---

Dodaci vam omogućavaju da proširite CrokCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja CrokCode.
...
- `.crokcode/plugins/` - Dodaci na nivou projekta
- `~/.config/crokcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/crokcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/crokcode/crokcode.json`)
2. Konfiguracija projekta (`crokcode.json`)
3. Globalni direktorij dodataka (`~/.config/crokcode/plugins/`)
4. Direktorij dodataka projekta (`.crokcode/plugins/`)
...
CrokCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Crokcode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje CrokCode.
...
Spriječite crokcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u crokcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji crokcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za crokcode zajedno sa ugrađenim alatima.
