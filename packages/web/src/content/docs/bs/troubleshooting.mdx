...
Da biste otklonili probleme s CrokCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\crokcode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `crokcode --log-level DEBUG`.

---

## Pohrana

CrokCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/crokcode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\crokcode`
...
CrokCode Desktop pokreće lokalni CrokCode server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `CrokCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/crokcode/crokcode.jsonc` (ili `~/.config/crokcode/crokcode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/crokcode/crokcode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\crokcode\crokcode.jsonc`
...
CrokCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/crokcode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\crokcode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga CrokCode mogao ponovo izgraditi.

1. Potpuno zatvorite CrokCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/crokcode`
- **Linux**: obrišite `~/.cache/crokcode` (ili pokrenite `rm -rf ~/.cache/crokcode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\crokcode`

3. Ponovo pokrenite CrokCode Desktop.

---

### Rješavanje problema sa vezom na serveru

CrokCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `crokcode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CROKCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CROKCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, CrokCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa CrokCode karakteristikama.

---

### Obavještenja se ne prikazuju

CrokCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za CrokCode u postavkama vašeg OS-a, i
...
1. Zatvorite CrokCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka CrokCode Desktop aplikacije):

- `crokcode.settings.dat` (URL zadanog servera za desktop)
- `crokcode.global.dat` i `crokcode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `crokcode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\crokcode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. CrokCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\crokcode`

2. Ponovo pokrenite CrokCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti CrokCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
CrokCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
