...
Možete dodati vanjske alate u CrokCode koristeći _Model Context Protocol_, ili MCP. CrokCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [CrokCode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
CrokCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, CrokCode će:
...
Ako server zahtijeva autentifikaciju, CrokCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `crokcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, CrokCode sigurno čuva tokene u `~/.local/share/crokcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u CrokCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz CrokCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete CrokCode sa vašim Sentry nalogom.
