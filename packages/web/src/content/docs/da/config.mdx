---
title: Konfiguration
description: Ved at bruge CrokCode JSON konfig.
---

Du kan konfigurere CrokCode ved hjælp af en JSON-konfigurationsfil.

---

## Format

CrokCode understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global config** (`~/.config/crokcode/crokcode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`CROKCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`crokcode.json` i projekt) - projektspecifikke indstillinger
5. **`.crokcode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`CROKCODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.crokcode` og `~/.config/crokcode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale CrokCode-konfiguration i `~/.config/crokcode/crokcode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `crokcode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når CrokCode starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `CROKCODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `CROKCODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.crokcode` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.crokcode` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan konfigurere serverindstillinger for kommandoerne `crokcode serve` og `crokcode web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din CrokCode-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `crokcode.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din CrokCode-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger CrokCode at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din CrokCode-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/crokcode/agents/` eller `.crokcode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil CrokCode falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`crokcode run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/crokcode/commands/` eller `.crokcode/commands/`. [Learn more here](/docs/commands).
...
CrokCode vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard crokcode **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide CrokCode med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.crokcode/plugins/` eller `~/.config/crokcode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse CrokCode til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
