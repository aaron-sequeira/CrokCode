...
Med CrokCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger CrokCode vårt eget `crokcode`-tema.
...
CrokCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/crokcode/themes/*.json` eller `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.crokcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.crokcode/themes/*.json`
