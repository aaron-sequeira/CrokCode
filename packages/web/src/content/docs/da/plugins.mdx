---
title: Plugins
description: Skriv dine egne plugins for at udvide CrokCode.
---

Plugins giver dig mulighed for at udvide CrokCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre CrokCodes standardadfærd.
...
- `.crokcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/crokcode/node_modules/`.
...
1. Global konfiguration (`~/.config/crokcode/crokcode.json`)
2. Projektkonfiguration (`crokcode.json`)
3. Global plugin-mappe (`~/.config/crokcode/plugins/`)
4. Projekt plugin bibliotek (`.crokcode/plugins/`)
...
CrokCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En crokcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide crokcode.
...
:::note
Hvis du bruger CrokCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå crokcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til crokcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som crokcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for crokcode sammen med indbyggede værktøjer.
