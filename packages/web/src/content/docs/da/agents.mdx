...
Der er to typer agenter i CrokCode; primære agenter og subagenter.
...
CrokCode leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
CrokCode leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

CrokCode leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i CrokCode's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `crokcode.json`-konfigurationsfil:
...
- Globalt: `~/.config/crokcode/agents/`
- Pr. projekt: `.crokcode/agents/`
...
Hvis ingen temperatur er angivet, bruger CrokCode modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale CrokCode-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din CrokCode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [OpenCode Zen](/docs/zen), vil du bruge `crokcode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `crokcode models` for at se en liste over de tilgængelige modeller.
:::
