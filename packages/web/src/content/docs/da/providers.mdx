---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i CrokCode.
---

import config from "../../../../config.mjs"
export const console = config.console

CrokCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i CrokCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/crokcode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i CrokCode-konfigurationen.
...
OpenCode Zen er en liste over modeller leveret af CrokCode-teamet, som er blevet testet og verificeret til at fungere godt med CrokCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg crokcode og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i CrokCode og er helt valgfrit at bruge.

---

## OpenCode Go

OpenCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af CrokCode-teamet, som er testet og verificeret til at fungere godt med CrokCode.
...
Sådan bruger du Amazon Bedrock med CrokCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører crokcode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `crokcode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i CrokCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere crokcode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at crokcode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem crokcode-konfigurationen.
...
gemme tokenet i crokcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
CrokCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse CrokCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`crokcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med CrokCode:
...
Sådan bruger du Google Vertex AI med CrokCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører crokcode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem crokcode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [crokcode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver CrokCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver CrokCode-samtale opført som en separat session.
...
Du kan konfigurere crokcode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere crokcode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere crokcode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for CrokCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med CrokCode:
...
3. Kopier API-nøglen til brug i CrokCode.
...
6. **Vigtigt**: Før du bruger skymodeller i CrokCode, skal du hente modelinformationen lokalt:
...
OpenCode Zen er en liste over testede og verificerede modeller leveret af CrokCode-teamet. [Læs mere](/docs/zen).
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem crokcode-konfigurationen.
...
5. Du kan også tilpasse dem gennem crokcode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CrokCode:
...
Du kan også tilpasse modeller gennem crokcode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med crokcode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `crokcode.json`-filen i projektkataloget:
...
`limit`-felterne lader CrokCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `crokcode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek crokcode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i crokcode-konfigurationen.
