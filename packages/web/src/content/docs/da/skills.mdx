...
Agentferdigheter lar CrokCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
CrokCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.crokcode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/crokcode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går CrokCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.crokcode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/crokcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.crokcode/skills/git-release/SKILL.md` slik:
...
CrokCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `crokcode.json`:
...
**For innebygde agenter** (i `crokcode.json`):
