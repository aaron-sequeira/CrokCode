---
title: Server
description: Interager med crokcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `crokcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en crokcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `crokcode.local` |
...
Indstil `CROKCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `crokcode`, eller indstil `CROKCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `crokcode serve` og `crokcode web`.
...
Når du kører `crokcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug crokcode-serveren til at interagere med crokcode programmatisk.
:::

Denne arkitektur lader crokcode understøtte flere klienter og lader dig interagere med crokcode programmatisk.

Du kan køre `crokcode serve` for at starte en selvstændig server. Hvis du har
crokcode TUI kørende, vil `crokcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af CrokCode [IDE](/docs/ide) plugins.
...
CrokCode-serveren viser følgende API'er.
