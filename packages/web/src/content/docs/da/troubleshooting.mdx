...
For at fejlfinde problemer med CrokCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\crokcode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `crokcode --log-level DEBUG`.

---

## Lagring

crokcode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/crokcode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\crokcode`
...
CrokCode Desktop kører en lokal CrokCode-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `CrokCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/crokcode/crokcode.jsonc` (eller `~/.config/crokcode/crokcode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/crokcode/crokcode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\crokcode\crokcode.jsonc`
...
CrokCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/crokcode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\crokcode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så CrokCode kan genopbygge den.

1. Afslut CrokCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/crokcode`
- **Linux**: slet `~/.cache/crokcode` (eller kør `rm -rf ~/.cache/crokcode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\crokcode`

3. Start CrokCode Desktop på ny.

---

### Løs problemer med serverforbindelse

CrokCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `crokcode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `CROKCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `CROKCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver CrokCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i CrokCode.

---

### Meddelelser vises ikke

CrokCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for CrokCode i dine OS-indstillinger, og
...
1. Afslut CrokCode Desktop.
2. Find og slet disse filer (de findes i CrokCode Desktop-appens datamappe):

- `crokcode.settings.dat` (skrivebordsstandardserver URL)
- `crokcode.global.dat` og `crokcode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med CrokCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `crokcode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\crokcode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. crokcode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\crokcode`

2. Start crokcode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge crokcode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
crokcode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
