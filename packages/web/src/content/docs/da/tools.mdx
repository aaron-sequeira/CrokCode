...
Her er alle de innebygde verktøyene som er tilgængelige i CrokCode.
...
:::note
Dette verktøyet er bare tilgængelig når `CROKCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `CROKCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger CrokCode-leverandøren eller når miljøvariabelen `CROKCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
