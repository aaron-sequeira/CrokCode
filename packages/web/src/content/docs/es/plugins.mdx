---
title: Complementos
description: Escriba sus propios complementos para extender CrokCode.
---

Los complementos le permiten extender CrokCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de CrokCode.
...
- `.crokcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/crokcode/plugins/` - Complementos globales
...
```json title="crokcode.json"
{
  ...
  "plugin": ["crokcode-helicone-session", "crokcode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/crokcode/node_modules/`.
...
1. Configuración global (`~/.config/crokcode/crokcode.json`)
2. Configuración del proyecto (`crokcode.json`)
3. Directorio global de complementos (`~/.config/crokcode/plugins/`)
4. Directorio de complementos del proyecto (`.crokcode/plugins/`)
...
```json title=".crokcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
CrokCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".crokcode/plugins/my-plugin.ts"
...
```js title=".crokcode/plugins/example.js"
...
- `client`: Un cliente SDK crokcode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar crokcode.
...
```js title=".crokcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "crokcode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio CrokCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que crokcode lea archivos `.env`:

```javascript title=".crokcode/plugins/env-protection.js"
...
```javascript title=".crokcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a crokcode:

```ts title=".crokcode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@crokcode/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que crokcode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para crokcode junto con las herramientas integradas.
...
```ts title=".crokcode/plugins/compaction.ts"
import type { Plugin } from "@crokcode/plugin"
...
```ts title=".crokcode/plugins/custom-compaction.ts"
import type { Plugin } from "@crokcode/plugin"
