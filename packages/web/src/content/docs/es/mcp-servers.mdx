...
Puede agregar herramientas externas a CrokCode utilizando el _Model Context Protocol_ o MCP. CrokCode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [CrokCode Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
CrokCode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, CrokCode:
...
Si el servidor requiere autenticación, CrokCode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `crokcode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, CrokCode almacenará los tokens de forma segura en `~/.local/share/crokcode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en CrokCode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración CrokCode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar CrokCode a su cuenta Sentry.
