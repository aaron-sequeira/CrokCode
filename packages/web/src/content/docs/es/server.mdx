---
title: Servidor
description: Interactuar con el servidor crokcode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `crokcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente crokcode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `crokcode.local` |
...
Configure `CROKCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `crokcode`, o configure `CROKCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `crokcode serve` como a `crokcode web`.
...
Cuando ejecuta `crokcode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor crokcode para interactuar con crokcode mediante programación.
:::

Esta arquitectura permite que crokcode admita múltiples clientes y le permite interactuar con crokcode mediante programación.

Puede ejecutar `crokcode serve` para iniciar un servidor independiente. Si tienes el
crokcode TUI ejecutándose, `crokcode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos CrokCode [IDE](/docs/ide).
...
El servidor crokcode expone las siguientes API.
