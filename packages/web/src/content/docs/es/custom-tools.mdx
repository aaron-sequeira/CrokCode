...
description: Cree herramientas que LLM pueda llamar en crokcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de crokcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.crokcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/crokcode/tools/`.
...
```ts title=".crokcode/tools/database.ts" {1}
...
```ts title=".crokcode/tools/math.ts"
import { tool } from "@crokcode/plugin"
...
```ts title=".crokcode/tools/bash.ts"
import { tool } from "@crokcode/plugin"
...
```ts title=".crokcode/tools/project.ts" {8}
import { tool } from "@crokcode/plugin"
...
```python title=".crokcode/tools/add.py"
import sys
...
```ts title=".crokcode/tools/python-add.ts" {10}
import { tool } from "@crokcode/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".crokcode/tools/add.py")
