...
Las herramientas permiten que LLM realice acciones en su código base. CrokCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en CrokCode.
...
:::note
Esta herramienta solo está disponible cuando `CROKCODE_EXPERIMENTAL_LSP_TOOL=true` (o `CROKCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor CrokCode o cuando la variable de entorno `CROKCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar CrokCode:

```bash
CROKCODE_ENABLE_EXA=1 crokcode
```
