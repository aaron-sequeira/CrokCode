...
Las habilidades del agente permiten a CrokCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
CrokCode busca estas ubicaciones:

- Configuración del proyecto: `.crokcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/crokcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, CrokCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.crokcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/crokcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.crokcode/skills/git-release/SKILL.md` así:
...
CrokCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `crokcode.json`:
...
**Para agentes integrados** (en `crokcode.json`):
