---
title: Proveedores
description: Usando cualquier proveedor LLM en CrokCode.
---

import config from "../../../../config.mjs"
export const console = config.console

CrokCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración CrokCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu CrokCode
configuración.
...
OpenCode Zen es una lista de modelos proporcionados por el equipo CrokCode que han sido
probado y verificado para funcionar bien con CrokCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con OpenCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione crokcode y diríjase a [opencode.ai/auth](https://opencode.ai/auth).
...
Funciona como cualquier otro proveedor en CrokCode y su uso es completamente opcional.

---

## OpenCode Go

OpenCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de CrokCode que han sido
probado y verificado para funcionar bien con CrokCode.
...
Para usar Amazon Bedrock con CrokCode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta crokcode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `crokcode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en CrokCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar crokcode para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que crokcode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración crokcode.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `CrokCode`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación crokcode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
CrokCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear CrokCode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `crokcode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `crokcode.json`:
...
Para utilizar su suscripción GitHub Copilot con crokcode:
...
Para utilizar Google Vertex AI con CrokCode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta crokcode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de crokcode, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación CrokCode enumerada como una sesión separada.
...
Puede configurar crokcode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar crokcode para usar modelos locales a través de LM Studio.
...
Puedes configurar crokcode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para CrokCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con CrokCode:
...
3. Copie la clave API para usarla en CrokCode.
...
6. **Importante**: Antes de usar modelos en la nube en CrokCode, debe obtener la información del modelo localmente:
...
OpenCode Zen es una lista de modelos probados y verificados proporcionada por el equipo CrokCode. [Más información](/docs/zen).
...
3. Ingrese su clave CrokCode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración crokcode.
...
5. También puedes personalizarlos a través de tu configuración crokcode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con CrokCode:
...
También puede personalizar modelos a través de su configuración crokcode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con crokcode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `crokcode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a CrokCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `crokcode auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración crokcode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración crokcode.
