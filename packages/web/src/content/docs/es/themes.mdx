...
Con CrokCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, CrokCode usa nuestro propio tema `crokcode`.
...
CrokCode viene con varios temas integrados.
...
- Quiere que CrokCode coincida con la apariencia de su terminal
...
CrokCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/crokcode/themes/*.json` o `$XDG_CONFIG_HOME/crokcode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.crokcode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.crokcode/themes/*.json`
