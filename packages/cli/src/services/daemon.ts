...
import { createCrokcodeClient } from "@crokcode/sdk/v2/client"
import { ServerAuth } from "@crokcode/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createCrokcodeClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@crokcode/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createCrokcodeClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createCrokcodeClient({ baseUrl: connection.url, headers: connection.headers })
    })
