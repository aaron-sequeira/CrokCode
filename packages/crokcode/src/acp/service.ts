...
import { InstallationVersion } from "@crokcode/core/installation/version"
import { AppNodeBuilder } from "@crokcode/core/effect/app-node-builder"
import type { AssistantMessage, Message, CrokcodeClient, SessionMessageResponse } from "@crokcode/sdk/v2"
...
export const AuthMethodID = "crokcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@crokcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `crokcode auth login` in the terminal",
      name: "Login with crokcode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: CrokcodeClient) {
...
function makeUsageService(sdk: CrokcodeClient) {
...
async function loadDirectorySnapshot(sdk: CrokcodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, crokcode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const crokcodeProvider = providers[ProviderV2.ID.make("opencode")]
  const crokcodeModel = crokcodeProvider ? Provider.sort(Object.values(crokcodeProvider.models))[0] : undefined
  if (crokcodeProvider && crokcodeModel) return { providerID: crokcodeProvider.id, modelID: crokcodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "CrokCode service failure", service })
}
