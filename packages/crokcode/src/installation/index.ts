...
import { InstallationChannel, InstallationVersion } from "@crokcode/core/installation/version"
import { NpmConfig } from "@crokcode/core/npm-config"
import { InstallationEvent } from "@crokcode/schema/installation-event"
...
export function userAgent(client = "cli") {
  return `crokcode/${InstallationChannel}/${InstallationVersion}/${client}`
}
...
export class Service extends Context.Service<Service, Interface>()("@crokcode/Installation") {}
...
const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getBrewFormula = Effect.fnUntraced(function* () {
      const tapFormula = yield* text(["brew", "list", "--formula", "anomalyco/tap/crokcode"])
      if (tapFormula.includes("opencode")) return "anomalyco/tap/crokcode"
      const coreFormula = yield* text(["brew", "list", "--formula", "opencode"])
      if (coreFormula.includes("opencode")) return "crokcode"
      return "crokcode"
    })
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        ...
        if (process.execPath.includes(path.join(".crokcode", "bin"))) return "curl" as Method
        ...
        for (const check of checks) {
          ...
          const installedName =
            check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "crokcode" : "opencode-ai"
      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "brew") {
          ...
          const response = yield* httpOk.execute(
            HttpClientRequest.get("https://formulae.brew.sh/api/formula/crokcode.json").pipe(
        ...
        if (detectedMethod === "choco") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27crokcode%27%20and%20IsLatestVersion&$select=Version",
        ...
        if (detectedMethod === "scoop") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/crokcode.json",
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "choco":
            ...
            break
          case "scoop":
            upgradeResult = yield* run(["scoop", "install", `crokcode@${target}`])
            break
          default:
            upgradeResult = yield* upgradeCurl(target)
            break
