...
import { CrokCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for crokcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(CrokCodeHttpApi)
...
export const PublicApi = CrokCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "crokcode",
    version: "1.0.0",
    description: "crokcode api",
