...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from crokcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const crokcode = await crokcodeFiles(input)
  for (const file of crokcode) {
...
async function crokcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "opencode"),
    ...(await Filesystem.findUp(["crokcode.json", "crokcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "opencode"))
  }
  if (Flag.CROKCODE_CONFIG) files.push(Flag.CROKCODE_CONFIG)
