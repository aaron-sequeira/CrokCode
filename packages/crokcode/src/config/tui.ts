...
import { TuiKeybind } from "@crokcode/tui/config/keybind"
...
import { TuiConfig } from "@crokcode/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@crokcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old crokcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.crokcode`
  // folders between cwd and home, and CROKCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CROKCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CROKCODE_TUI_CONFIG override, if set.
  if (Flag.CROKCODE_TUI_CONFIG) {
    const configFile = Flag.CROKCODE_TUI_CONFIG
  ...
  // 4. `.crokcode` directories (and CROKCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".crokcode") || dir === Flag.CROKCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".crokcode") && dir !== Flag.CROKCODE_CONFIG_DIR) continue
