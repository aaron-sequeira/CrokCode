...
export async function checkPluginCompatibility(target: string, crokcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(crokcodeVersion) || semver.major(crokcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.crokcode
  if (typeof range !== "string") return
  if (!semver.satisfies(crokcodeVersion, range)) {
    throw new Error(`Plugin requires crokcode ${range} but running ${crokcodeVersion}`)
  }
}
