...
Practical reference for a future `packages/server` split after the crokcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/crokcode`.
...
Until enough shared service code lives outside `packages/crokcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/crokcode`

It should not import `packages/crokcode` services while `packages/crokcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on crokcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/crokcode` implementation details.
