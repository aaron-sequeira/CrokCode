...
- Author package entrypoint is `@crokcode/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/crokcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@crokcode/plugin/tui`.
- `@crokcode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@crokcode/plugin/tui"
...
- The value is a semver range checked against the running CrokCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.crokcode` is absent, no check is performed (backward compatible).
...
- `crokcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `crokcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.crokcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.crokcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.crokcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.crokcode` dir or `CROKCODE_CONFIG_DIR`, CrokCode installs `@crokcode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@crokcode/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus CrokCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
CrokCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, CrokCode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@crokcode/plugin/tui`.
...
- `message` is required; `title` defaults to `"crokcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `crokcode.default`.
...
- Local plugins persist installed themes under the local `.crokcode/themes` area near the plugin config source.
...
- `--pure` / `CROKCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.crokcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.crokcode/plugins/tui-vim.tsx`
- Local smoke config: `.crokcode/tui.json`
- Local smoke theme: `.crokcode/plugins/smoke-theme.json`
