...
Delete `packages/crokcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/crokcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/crokcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/crokcode`
- TUI plugin loader tests in `packages/crokcode` if runtime plugin API wiring changed
