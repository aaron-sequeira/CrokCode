...
Trim `packages/crokcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/crokcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/crokcode`.
- `[x]` `bun typecheck` from `packages/crokcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/crokcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/crokcode`.
- `[x]` `bun dev generate > /tmp/crokcode-openapi.json` from `packages/crokcode`.
...
- Edit `packages/crokcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/crokcode-openapi.json` from `packages/crokcode`.
...
- Check source schemas in files like `packages/crokcode/src/session/schema.ts`, `packages/crokcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/crokcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/crokcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
