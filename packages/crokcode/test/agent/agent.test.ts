...
it.instance("plan agent denies edits except .crokcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".crokcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".crokcode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CROKCODE_TEST_HOME
      process.env.CROKCODE_TEST_HOME = test.directory
