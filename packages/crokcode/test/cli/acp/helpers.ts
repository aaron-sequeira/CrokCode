...
export function createAcpClient(input: Pick<CliFixture, "crokcode">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.crokcode.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "crokcode-local-acp", version: "0.1.0" },
      }),
    )
  })
}
