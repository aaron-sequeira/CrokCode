...
import type { QuestionRequest } from "@crokcode/sdk/v2"
import { CrokcodeKeymapProvider, registerCrokcodeKeymap } from "@crokcode/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerCrokcodeKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerCrokcodeKeymap(keymap, renderer, tuiConfig)
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerCrokcodeKeymap(keymap, renderer, tuiConfig)
