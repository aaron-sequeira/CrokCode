...
import { CrokcodeClient, type GlobalEvent } from "@crokcode/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<CrokcodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<CrokcodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<CrokcodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<CrokcodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<CrokcodeClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: CrokcodeClient["event"]["subscribe"]
    globalEvent?: CrokcodeClient["global"]["event"]
    promptAsync?: CrokcodeClient["session"]["promptAsync"]
    status?: CrokcodeClient["session"]["status"]
    messages?: CrokcodeClient["session"]["messages"]
    children?: CrokcodeClient["session"]["children"]
    permissions?: CrokcodeClient["permission"]["list"]
    questions?: CrokcodeClient["question"]["list"]
  } = {},
) {
  const client = new CrokcodeClient()

  const subscribe: CrokcodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: CrokcodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: CrokcodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: CrokcodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: CrokcodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: CrokcodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: CrokcodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: CrokcodeClient["question"]["list"] = input.questions ?? (() => ok([]))
