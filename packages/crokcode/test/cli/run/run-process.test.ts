// Subprocess integration tests for `crokcode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `crokcode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `CROKCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("crokcode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, crokcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* crokcode.run("say hi")
        crokcode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, crokcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* crokcode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, crokcode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* crokcode.run("think", { extraArgs: ["--thinking"] })
        crokcode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* crokcode.run("think again")
        crokcode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, crokcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* crokcode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, crokcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* crokcode.run("say hi", { format: "json" })
        crokcode.expectExit(result, 0)

        const events = crokcode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ crokcode }) =>
      Effect.gen(function* () {
        const result = yield* crokcode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, crokcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* crokcode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, crokcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* crokcode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, crokcode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* crokcode.run("request permission", { permission: { bash: "ask" } })
        crokcode.expectExit(denied, 0)
        ...
        const allowed = yield* crokcode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        crokcode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* crokcode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        crokcode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, crokcode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* crokcode.serve()

        const result = yield* crokcode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, crokcode }) =>
      Effect.gen(function* () {
        const result = yield* crokcode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, crokcode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* crokcode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
