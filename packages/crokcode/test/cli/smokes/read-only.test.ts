...
describe("crokcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CROKCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ crokcode }) =>
      Effect.gen(function* () {
        const r = yield* crokcode.spawn(["mcp", "list"])
        crokcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ crokcode }) =>
      Effect.gen(function* () {
        const r = yield* crokcode.spawn(["providers", "list"])
        crokcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ crokcode }) =>
      Effect.gen(function* () {
        const r = yield* crokcode.spawn(["models"])
        crokcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ crokcode }) =>
      Effect.gen(function* () {
        const r = yield* crokcode.spawn(["agent", "list"])
        crokcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CROKCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ crokcode }) =>
      Effect.gen(function* () {
        const r = yield* crokcode.spawn(["session", "list"])
        crokcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ crokcode }) =>
      Effect.gen(function* () {
        const r = yield* crokcode.spawn(["stats"])
        crokcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CROKCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ crokcode }) =>
      Effect.gen(function* () {
        const r = yield* crokcode.spawn(["db", "path"])
        crokcode.expectExit(r, 0, "db path")
