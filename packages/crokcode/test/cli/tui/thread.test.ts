...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ crokcode }) =>
    Effect.gen(function* () {
      const result = yield* crokcode.spawn(["--replay-limit", "10"])

      crokcode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ crokcode }) =>
    Effect.gen(function* () {
      const result = yield* crokcode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ crokcode }) =>
    Effect.gen(function* () {
      const result = yield* crokcode.spawn(["--mini", "--port", "4096"])
