...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/crokcode.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/crokcode.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/crokcode.jsonc", dir: ".crokcode", suggestion: "crokcode" },
        expected:
          'Directory ".crokcode" in /tmp/crokcode.jsonc is not valid. Rename the directory to "crokcode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/crokcode.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/crokcode.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
