// Subprocess test harness for the crokcode CLI. Spawns the real binary against
...
//
// Configuration flows through crokcode's built-in test affordances:
//   - CROKCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - CROKCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - CROKCODE_DISABLE_PROJECT_CONFIG : skip walking up for crokcode.json
//   - CROKCODE_PURE                : skip external plugin discovery + install
//   - CROKCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `crokcode.run` is fully wired. The shape supports adding more
// builders (`crokcode.serve(opts)`, `crokcode.acp(opts)`, `crokcode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of CrokcodeCli.
...
const crokcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(crokcodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CROKCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CROKCODE_CONFIG_CONTENT: configJson,
    CROKCODE_DISABLE_PROJECT_CONFIG: "1",
    CROKCODE_PURE: "1",
    CROKCODE_DISABLE_AUTOUPDATE: "1",
    CROKCODE_DISABLE_AUTOCOMPACT: "1",
    CROKCODE_DISABLE_MODELS_FETCH: "1",
    CROKCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `crokcode run`. New flags should
...
// `crokcode serve` is a long-lived process — it never exits on its own.
...
// `crokcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type CrokcodeCli = {
  ...
  // Spawn `crokcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `crokcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("crokcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CROKCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("crokcode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("crokcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "crokcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `crokcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("crokcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const crokcode: CrokcodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, crokcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `crokcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "crokcode") {
...
// every call site. Use this for any test that needs the crokcode CLI fixture.
//
...
// resources (e.g. `crokcode.serve`) without an extra `Effect.scoped` wrapper —
