...
describe("ServerAuth", () => {
  ...
  test("defaults to the crokcode username", () => {
    Flag.CROKCODE_SERVER_PASSWORD = "secret"
    Flag.CROKCODE_SERVER_USERNAME = undefined

    expect(ServerAuth.headers()).toEqual({
      Authorization: `Basic ${Buffer.from("crokcode:secret").toString("base64")}`,
    })
  })
  ...
  test("validates decoded credentials against effect config", () => {
    ...
    expect(ServerAuth.authorized({ username: "crokcode", password: Redacted.make("secret") }, config)).toBe(false)
  })
})
