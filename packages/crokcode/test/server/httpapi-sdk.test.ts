...
import { createCrokcodeClient } from "@crokcode/sdk/v2"
...
type Sdk = ReturnType<typeof createCrokcodeClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.CROKCODE_SERVER_PASSWORD = input?.password
      Flag.CROKCODE_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".crokcode", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-crokcode-directory")).toBe(false)
        expect(request!.headers.has("x-crokcode-workspace")).toBe(false)
      }),
    ),
  )
