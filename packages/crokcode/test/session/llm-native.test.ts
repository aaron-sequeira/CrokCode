...
import { LLMEvent, ToolFailure } from "@crokcode/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@crokcode/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, crokcode, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored crokcode auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("opencode") },
        provider: {
