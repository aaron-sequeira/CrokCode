...
import type { HostPluginApi } from "@crokcode/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createCrokcodeClient({
  ...
  let selected = opts.theme?.selected ?? "crokcode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "crokcode.default",
