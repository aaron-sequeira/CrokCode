...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "crokcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "crokcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "crokcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "crokcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["crokcode.json", "crokcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "crokcode.json"),
        path.join(project, "crokcode.jsonc"),
      ])
    })
  })
