...
import { Config } from "@crokcode/core/config"
import { ConfigProvider } from "@crokcode/core/config/provider"
import { AppNodeBuilder } from "@crokcode/core/effect/app-node-builder"
import { LayerNode } from "@crokcode/core/effect/layer-node"
import { ConfigMigrateV1 } from "@crokcode/core/v1/config/migrate"
...
import { Policy } from "@crokcode/core/policy"
import { Project } from "@crokcode/core/project"
...
describe("Config", () => {
  ...
  it.live("loads crokcode JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "crokcode.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "crokcode.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "crokcode.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "crokcode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "crokcode.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "crokcode.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "crokcode.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .crokcode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".crokcode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".crokcode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "crokcode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "crokcode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "crokcode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "crokcode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "crokcode.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".crokcode", "crokcode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".crokcode", "crokcode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".crokcode")),
              AbsolutePath.make(path.join(directory, ".crokcode")),
            ])
