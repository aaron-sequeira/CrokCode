...
import { ModelsDevPlugin } from "@crokcode/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.CROKCODE_MODELS_PATH,
          disabled: Flag.CROKCODE_DISABLE_MODELS_FETCH,
        }
        Flag.CROKCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.CROKCODE_MODELS_PATH = previous.path
          Flag.CROKCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
