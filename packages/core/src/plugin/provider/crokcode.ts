...
import type { IntegrationOAuthMethodRegistration } from "@crokcode/plugin/v2/effect/integration"
import { define } from "@crokcode/plugin/v2/effect/plugin"
import type { CredentialValue } from "@crokcode/sdk/v2/types"
...
export const CrokcodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "opencode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("CrokcodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load CrokCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("opencode", (integration) => {
        integration.name = "CrokCode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "crokcode", method: { type: "key", label: "API key (service account)" } })
    })
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      const item = catalog.provider.get(ProviderV2.ID.crokcode)
      if (!item) return
      const hasKey = Boolean(process.env.CROKCODE_API_KEY || connected || item.provider.request.body.apiKey)
