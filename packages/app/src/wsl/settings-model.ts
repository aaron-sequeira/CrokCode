...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-crokcode" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslCrokcodeAction(check?: WslCrokcodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install CrokCode"
  if (check.matchesDesktop === false) return "Update CrokCode"
}
...
export function addServerViewModel(input: {
  ...
  const crokcodeCheck = selectedDistro ? (state?.crokcodeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.crokcodeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateCrokcode" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.crokcodeMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installCrokcode" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  crokcodeCheck: WslCrokcodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingCrokcode =
    input.state?.job?.kind === "install-crokcode" && input.state.job.distro === input.selectedDistro
  if (!ready || probingCrokcode) {
    return {
      variant: "contrast",
  ...
  if (!addServerCrokcodeReady(input.crokcodeCheck)) {
    const update = !!input.crokcodeCheck?.resolvedPath && input.crokcodeCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingCrokcode
        ? { key: "wsl.onboarding.updatingCrokcode" }
        : update
          ? { key: "wsl.onboarding.updateCrokcode" }
          : { key: "wsl.onboarding.installCrokcode" },
      disabled: !!input.state?.job || input.adding,
      action: "install-crokcode",
      loading: installingCrokcode,
...
function addServerCrokcodeReady(check: WslCrokcodeCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.crokcodeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.crokcodeChecks[item.name]) return [`crokcode:${item.name}`]
    return []
  })
