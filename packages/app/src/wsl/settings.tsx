...
import { Tag } from "@crokcode/ui/v2/badge-v2"
...
import { wslCrokcodeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.crokcodeChecks[item.config.distro]
          const crokcodeAction = () => wslCrokcodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-crokcode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={crokcodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installCrokcode(item.config.distro))}
                    >
