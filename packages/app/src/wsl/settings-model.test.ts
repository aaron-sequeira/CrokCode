...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when CrokCode needs attention", () => {
    expect(wslCrokcodeAction(undefined)).toBeUndefined()
    expect(
      wslCrokcodeAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install CrokCode")
    expect(
      wslCrokcodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/crokcode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update CrokCode")
    expect(
      wslCrokcodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/crokcode",
  ...
  test("does not report ready when CrokCode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installCrokcode" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-crokcode")
  })
