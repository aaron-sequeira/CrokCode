import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@crokcode/sdk/v2/client"
import { Button } from "@crokcode/ui/button"
import { useDialog } from "@crokcode/ui/context/dialog"
import { Dialog } from "@crokcode/ui/dialog"
import { Icon } from "@crokcode/ui/icon"
import { IconButton } from "@crokcode/ui/icon-button"
import { List, type ListRef } from "@crokcode/ui/list"
import { ProviderIcon } from "@crokcode/ui/provider-icon"
import { Spinner } from "@crokcode/ui/spinner"
import { Tag } from "@crokcode/ui/tag"
import { TextField } from "@crokcode/ui/text-field"
import { ButtonV2 } from "@crokcode/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@crokcode/ui/v2/dialog-v2"
import { TextInputV2 } from "@crokcode/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "opencode-go") return language.t("dialog.provider.crokcodeGo.tagline")
    return undefined
  }
      ...
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <Show when={i.id === "opencode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.crokcode.tagline")}</div>
          </Show>
                          ...
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "opencode"
                                ? "dialog.provider.crokcode.tagline"
                                : "dialog.provider.crokcodeGo.tagline",
                            )}
                          </span>
        ...
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          ...
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.crokcodeZen.line1")}</div>
              <div>{language.t("provider.connect.crokcodeZen.line2")}</div>
              <div>
                {language.t("provider.connect.crokcodeZen.visit.prefix")}
                <Link
                  href="https://opencode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.crokcodeZen.visit.link")}
                </Link>
                {language.t("provider.connect.crokcodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
        ...
        <Switch>
          <Match when={provider().id === "opencode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.crokcodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.crokcodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.crokcodeZen.visit.prefix")}
