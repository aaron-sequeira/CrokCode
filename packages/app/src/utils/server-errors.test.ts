...
import type { SessionNotFoundError } from "@crokcode/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em crokcode.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
