...
const directory = "C:/CrokCode/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#crokcode-titlebar-right")
...
async function configurePage(page: Page) {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem("crokcode.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
