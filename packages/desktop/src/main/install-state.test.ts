...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier CrokCode launch", () => {
    expect(hasExistingAppState([file("crokcode.settings")])).toBe(true)
    expect(hasExistingAppState([file("crokcode.global.dat")])).toBe(true)
