import type { WslDistroProbe, WslCrokcodeCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  crokcodeChecks: Record<string, WslCrokcodeCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextCrokcodeChecks = { ...crokcodeChecks }
  delete nextDistroProbes[distro]
  delete nextCrokcodeChecks[distro]
  return { distroProbes: nextDistroProbes, crokcodeChecks: nextCrokcodeChecks }
}
