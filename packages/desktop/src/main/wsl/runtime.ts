...
export async function installWslCrokcode(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
...
export async function resolveWslCrokcode(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
