...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-crokcode", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installCrokcode(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-crokcode", unavailable)
