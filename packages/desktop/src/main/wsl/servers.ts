...
import { expectCrokcodeVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveCrokcode?: typeof resolveWslCrokcode
...
export function createWslServersController(
  ...
  const setCrokcodeCheck = (distro: string, check: WslCrokcodeCheck) => {
    setState({
      crokcodeChecks: {
        ...state.crokcodeChecks,
        [distro]: check,
      },
    })
  }

  const checkCrokcode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveCrokcode ?? resolveWslCrokcode)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return crokcodeCheck(distro, resolved, version, appVersion)
  }

  const refreshCrokcodeCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setCrokcodeCheck(distro, await checkCrokcode(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const crokcodeChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.crokcodeChecks[distro])
        .map(async (distro) => [distro, await checkCrokcode(distro, opts)] as const),
    )
    if (crokcodeChecks.length) {
      setState({ crokcodeChecks: { ...state.crokcodeChecks, ...Object.fromEntries(crokcodeChecks) } })
    }
  }
  ...
  const refreshCrokcodeCheckBackground = (id: string, distro: string) => {
    void checkCrokcode(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setCrokcodeCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl crokcode check failed", { id, distro, message })
      })
  }

  const refreshCrokcodeChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkCrokcode(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setCrokcodeCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl crokcode check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshCrokcodeCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installCrokcode(name: string) {
      await runJob({ kind: "install-crokcode", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslCrokcode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "CrokCode installation failed")
        }
        await refreshCrokcodeCheck(name, { signal: abort.signal })
        expectCrokcodeVersion(state.crokcodeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.crokcodeChecks, distro) : {}),
      })
    },
...
function crokcodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "crokcode is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "crokcode is installed but could not run",
    }
  }
  return {
    distro,
