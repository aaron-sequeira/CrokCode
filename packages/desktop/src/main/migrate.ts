...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.crokcode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `crokcode.settings.dat` is special: it maps to the `crokcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"crokcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // crokcode.settings.dat → the electron settings store ("crokcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "crokcode.global.dat").
  const storeName = filename === "crokcode.settings.dat" ? "crokcode.settings" : filename
