...
import type { UpdaterState } from "@crokcode/app/updater"
...
import "./styles.css"
import { Splash } from "@crokcode/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.CROKCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "crokcode:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__CROKCODE__ ??= {}
  const pending = window.__CROKCODE__.deepLinks ?? []
  window.__CROKCODE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `crokcode.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("crokcode.global.dat").getItem("language")
