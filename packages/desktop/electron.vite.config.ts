...
const channel = (() => {
  ...
  if (process.env.CROKCODE_CHANNEL === "latest") return "prod"
  return "dev"
})()
...
export default defineConfig({
  main: {
    define: {
      "import.meta.env.CROKCODE_CHANNEL": JSON.stringify(channel),
    },
    build: {
      rollupOptions: {
      ...
      },
      {
        ...
        async writeBundle() {
          for (const l of await fs.readdir(CROKCODE_SERVER_DIST)) {
            ...
            await fs.writeFile(`./out/main/chunks/${l}`, await fs.readFile(`${CROKCODE_SERVER_DIST}/${l}`))
          }
        },
      },
    ],
