...
import { createCrokcode, type ToolPart } from "@crokcode/sdk"
...
console.log("🚀 Starting crokcode server...")
const crokcode = await createCrokcode({
  port: 0,
})
console.log("✅ Crokcode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await crokcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new crokcode session...")
    const { client, server } = crokcode
    ...
    console.log("✅ Created crokcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to crokcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Crokcode response:", JSON.stringify(result, null, 2))
