...
This is the living design and status document for `@crokcode/codemode` and its existing V2 CrokCode adapter.
...
`packages/crokcode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
